...
import type { Assessment, DateRange, PearsonResult, Recording, TTestResult } from '../types';
...
/**
 * Builds an export-ready dataset of every non-archived recording (whether or
 * not it has an assessment attached), optionally limited to a date range.
 * Used by the "Export All Recordings" report action.
 */
export async function fetchRecordingsDataset(dateRange?: DateRange): Promise<RecordingDatasetRow[]> {
  let query = supabase
    .from('recordings')
    .select('*, device:devices(*)')
    .eq('is_archived', false)
    .order('started_at', { ascending: false });
  if (dateRange?.from) query = query.gte('started_at', dateRange.from);
  if (dateRange?.to) query = query.lte('started_at', `${dateRange.to}T23:59:59`);
...
export async function fetchAnalysisDataset(filters: AnalysisFilters = {}): Promise<AnalysisRow[]> {
  ...
  if (filters.assessmentNumber) query = query.eq('assessment_number', filters.assessmentNumber);
