...
- **Subjects, Sections, and Assessment Types are configurable**, not hardcoded. They live in the `settings_options` table and are managed from the **System Settings** page; the Assessment form and all filters read from this list.
- **One assessment per recording.** Adding assessment details is guarded against duplicate/rapid-click submissions — a recording can only have one active (non-archived) assessment attached at a time.
